...
function api_stats_get($request) {
  ...
  if ($user_id === 0) {
    $response = new WP_Error('error', 'User not found or without permission.', ['status' => 401]);
  ...
  if ($posts) {
    foreach ($posts as $post) {
      $stats[] = [
        'id' => $post->ID,
        //may also return image thumbnail for presentation. 
...
function register_api_stats_get() {
  register_rest_route('wh-api-v001', '/stats', [
